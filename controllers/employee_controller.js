...
//             return res.status(400).json({
//                 message:'password and cpassword are not the same',
//                 status:'failure',
//                 data:[]
//             });
//         }
...
//         if(!existingCompany){
//             return res.status(400).json({
//                 message:`${company} Company is not registerd`,
//                 status:'failure',
//                 data:[]
//             });
//         }

//         // Create and store user inside db
...
//         await Company.findByIdAndUpdate(existingCompany._id,{$push:{'employee':user._id}});
//         return res.status(200).json({
//             message:`Successfully created employee and added into ${company} company`,
//             status:'successful',
//             data:[{
//                     uid:user._id,
//                     cid:existingCompany._id
//                 }]
//         });
...
module.exports.employeeReview = async function (req, res) {
    try {
        ...
        const employee = await User.findById(employeeId)
            .select('-password').populate({ path: 'company', populate: { path: 'employees' } })
            .populate({ path: 'feedbackReceived', populate: { path: 'sender', select: 'name' } });
...
module.exports.askFeedback = async function (req, res) {
    try {
        const user = req.user;
        const { receiverId, giverId } = req.body;
        ...
        if (!receiver || !giver || !receiver.company.equals(user.company) || !giver.company.equals(user.company) || receiver.adminRank < user.adminRank || giver.adminRank < user.adminRank) {
        ...
        await User.findByIdAndUpdate(giverId, { $push: { feedbackPending: receiverId } });
...
module.exports.cancelFeedback = async function (req, res) {
    try {
        ...
        await User.findByIdAndUpdate(giverId, { $pull: { feedbackPending: receiverId } });
...
module.exports.submitFeedback = async function (req, res) {
    ...
    try {
        ...
        // checking if user is allowed to give feedback
        if (!req.user.feedbackPending.includes(receiverId)) {
            return res.status(401).json({
                message: 'User is not Authorized',
                status: 'failure',
                data: []
        ...
        // removing pending feedback id from sender
        const user = await User.findByIdAndUpdate(req.user.id, { $pull: { 'feedbackPending': receiverId } });

        // updating recievers rating and append feedback id to it
        const newRating = ((receiver.rating * receiver.feedbackReceived.length) + parseInt(rating)) / (receiver.feedbackReceived.length + 1);
        await User.findByIdAndUpdate(receiverId, { 'rating': newRating, $push: { 'feedbackReceived': feedback.id } })
...
module.exports.deleteEmployee = async function (req, res) {
    ...
    try {
        ...
        // deleting recieved feedbacks
        await Feedback.deleteMany({ _id: { $in: employee.feedbackReceived } });
        ...
        for (let index = 0; index < submitedFeedbacks.length; index++) {
            ...
            const sFeedback = await Feedback.findById(submitedFeedbacks[index]);
            let receivedEmployee = await User.findById(sFeedback.receiver);
            let newRating = undefined;

            if (receivedEmployee.feedbackReceived.length == 1) {
                newRating = 0;
            } else {
                newRating = ((receivedEmployee.rating * receivedEmployee.feedbackReceived.length) - parseInt(sFeedback.rating)) / (receivedEmployee.feedbackReceived.length - 1);
            }


            receivedEmployee.rating = newRating;
            receivedEmployee.feedbackReceived.pull(sFeedback._id);

            await receivedEmployee.save();
